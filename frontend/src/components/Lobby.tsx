  ...
  return (
    <div className="grid two">
      <section className="card stack">
        ...
        <ul className="room-list">{participants.map((participant) => <li key={participant.participantId}><div className="row"><img className="avatar" src={participant.avatar.kind === "uploaded" ? participant.avatar.dataUrl : avatarDataUrl(participant.avatar.seed)} alt="" /><div><strong>{participant.nickname}</strong><p className="muted">{participant.participantId === room.hostId ? "Host · " : ""}{participant.controlledCharacterCount} {locale === "uk" ? "персонаж(і)" : "character(s)"}</p></div></div><span className={`badge ${participant.ready ? "live" : ""}`}>{!participant.connected ? (locale === "uk" ? "зв’язок…" : "reconnecting") : participant.ready ? (locale === "uk" ? "готовий" : "ready") : (locale === "uk" ? "очікує" : "waiting")}</span></li>)}</ul>
...
function SettingsPanel({ room, editable, command }: { room: RoomSnapshot; editable: boolean; command: (name: "room:update-settings", body: Record<string, unknown>, success: string) => Promise<void> }) {
  ...
  return <aside className="card stack"><div className="row between"><h2>{locale === "uk" ? "Налаштування" : "Settings"}</h2>{!editable && <span className="badge">Host only</span>}</div>
    <label>{locale === "uk" ? "Максимум учасників" : "Maximum participants"}<input disabled={!editable} type="number" inputMode="numeric" min={3} max={15} value={settings.maxParticipants} onChange={(event) => setSettings({ ...settings, maxParticipants: Number(event.target.value) })} /></label>
    <label className="row"><input disabled={!editable} type="checkbox" checked={settings.fillToSix} onChange={(e) => setSettings({ ...settings, fillToSix: e.target.checked })} />Fill to six</label>
    <div className="grid two"><label>Final<select disabled={!editable} value={settings.mode} onChange={(e) => setSettings({ ...settings, mode: e.target.value as typeof settings.mode })}><option value="base">Base</option><option value="survival-story">Survival Story</option></select></label><label>Goal<select disabled={!editable} value={settings.finalGoal} onChange={(e) => setSettings({ ...settings, finalGoal: e.target.value as typeof settings.finalGoal })}><option value="salvation">Salvation</option><option value="revival">Revival</option></select></label></div>
    <div className="timer-grid">{timerNames.map((name) => <label key={name}>{name}<select disabled={!editable} value={settings.timers[name] ?? "off"} onChange={(e) => setSettings({ ...settings, timers: { ...settings.timers, [name]: e.target.value === "off" ? null : Number(e.target.value) } })}><option value="off">off</option><option value="30">30 s</option><option value="60">60 s</option><option value="120">120 s</option></select></label>)}</div>
    ...
    {editable && <button onClick={() => void command("room:update-settings", { settings }, locale === "uk" ? "Збережено; готовність скинуто" : "Saved; readiness reset")}>{locale === "uk" ? "Зберегти правила" : "Save rules"}</button>}
  </aside>;
}
